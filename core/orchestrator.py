# ============================================================
# UltraAgent — core/orchestrator.py  (FIXED VERSION)
# ============================================================
...
اكتب خطة تنفيذ كـ list مرقمة. كل سطر: [agent_name]: [وصف المهمة]"""
    ...
    meta["planning_model"] = model if model else ""
...
def routing(state: AgentState) -> dict:
    ...
    # ✅ FIX: لازم نرجع field واحد على الأقل — مش {} فاضية
    return {"current_step": current_step, "metadata": meta}
...
def executing(state: AgentState) -> dict:
    ...
    if not step:
        # ✅ FIX: بدل {} فاضية
        return {"results": dict(state.get("results") or {})}
...
def check_result(state: AgentState) -> dict:
    # ✅ FIX: بدل {} فاضية — LangGraph بيطلب field واحد على الأقل
    return {"error_count": state.get("error_count", 0)}
...
def run_task(task: str, chat_id: str = "default") -> str:
    ...
    try:
        meta = final_state["metadata"] if isinstance(final_state, dict) else final_state.metadata
        return meta.get("final_response", "عذراً، حدث خطأ.")
    except Exception:
